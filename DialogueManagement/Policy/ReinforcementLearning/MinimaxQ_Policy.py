...
class MinimaxQ_Policy(Policy.Policy):
    def __init__(self, ontology, database, agent_id=0, agent_role='system', alpha=0.25, gamma=0.95, epsilon=0.25,
        ...
        if isinstance(database, DataBase):
            ...
            raise ValueError('MinimaxQ Policy: Unacceptable database type %s ' % database)
    ...
    def next_action(self, state):
        ...
        if state_enc not in self.pi or (self.is_training and random.random() < self.epsilon):
            if not self.is_training:
                if not self.pi:
                    print(f'\nWARNING! Minimax Q {self.agent_role} matrix is empty. Did you load the correct file?\n')
                else:
                    print(f'\nWARNING! Minimax {self.agent_role} state not found in Q matrix.\n')
        ...
        # other_action_enc = self.encode_action(state.user_acts, self.agent_role != 'system')
        # maxQ = max(self.Q[state_enc][other_action_enc][:-1])    # Do not consider 'UNK' etc
        # maxima = [i for i, j in enumerate(self.Q[state_enc][other_action_enc]) if j == maxQ]

        # sys_acts = self.decode_action(random.choices([a for a in range(self.NActions)], self.pi[state_enc])[0])
        maxPi = max(self.pi[state_enc])
        maxima = [i for i, j in enumerate(self.pi[state_enc]) if j == maxPi]
        ...
        if self.agent_role == 'user':
            ...
            if state.user_goal:
                ...
                for c in self.informable_slots:
                    if c != 'name':
                        if c in state.user_goal.constraints and c not in state.user_goal.actual_constraints:
                            found_unanswered_constr = True
                            break

                for r in self.requestable_slots:
                    if r in state.user_goal.requests and not state.user_goal.requests[r].value:
                        found_unanswered_req = True
                        break

                temp += [int(found_unanswered_constr), int(found_unanswered_req)]
            else:
                temp += [0, 0]
    ...
    def encode_action(self, actions, system=True):
        ...
        if system:
            ...
            if action.intent == 'inform':
                return len(self.dstc2_acts_usr) + len(self.requestable_slots) + self.system_requestable_slots.index(
                    action.params[0].slot)

        if (self.agent_role == 'system') == system:
            print(
                'WoLF-PHC ({0}) policy action encoder warning: Selecting default action (unable to encode: {1})!'.format(
                    self.agent_role, action))
        else:
            print(
                'WoLF-PHC ({0}) policy action encoder warning: Selecting default action (unable to encode other agent action: {1})!'.format(
                    self.agent_role, action))
        ...
        if system:
            ...
            if action_enc < len(self.dstc2_acts_usr) + len(self.requestable_slots) + len(self.system_requestable_slots):
                return [DialogueAct('inform',
                                    [DialogueActItem(self.system_requestable_slots[
                                                         action_enc - len(self.dstc2_acts_usr) - len(
                                                             self.requestable_slots)], Operator.EQ, '')])]

        # Default fall-back action
        print('WoLF-PHC Policy ({0}) policy action decoder warning: Selecting repeat() action (index: {1})!'.format(
    ...
    def train(self, dialogues):
        ...
        for dialogue in dialogues:
            ...
            for turn in dialogue:
                ...
                other_action_enc = self.encode_action(turn['state'].user_acts, self.agent_role != 'system')

                if action_enc < 0 or other_action_enc < 0 or turn['action'][0].intent == 'bye':
                    continue
                ...
                self.Q[state_enc][other_action_enc][action_enc] += self.alpha * delta
        ...
        print('MiniMaxQ [alpha: {0}, epsilon: {1}]'.format(self.alpha, self.epsilon))
