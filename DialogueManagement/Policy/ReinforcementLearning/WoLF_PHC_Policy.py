...
from .. import Policy, HandcraftedPolicy, Calculated_Policy
...
class WoLF_PHC_Policy(Policy.Policy):
    def __init__(self, ontology, database, agent_id=0, agent_role='system', alpha=0.25, gamma=0.95, epsilon=0.25,
        ...
        if self.agent_role == 'system':
            ...
            # self.warmup_policy = Calculated_Policy.Calculated_Policy(self.ontology, self.database,
            #                                                          agent_id=self.agent_id, agent_role=self.agent_role)
            # self.warmup_policy.initialize(**{'policy_path': 'Models/CamRestPolicy/sys_policy.pkl'})
        ...
        # self.dstc2_acts_sys = ['welcomemsg', 'offer', 'bye']
        ...
        if self.dstc2_acts_sys:
            if self.agent_role == 'system':
                ...
                self.NOtherActions = len(self.dstc2_acts_usr) + len(self.requestable_slots) + len(self.system_requestable_slots)
            elif self.agent_role == 'user':
                # self.NActions = 4
                # self.NOtherActions = 5
                self.NActions = len(self.dstc2_acts_usr) + len(self.requestable_slots) + len(self.system_requestable_slots)
                self.NOtherActions = len(self.dstc2_acts_sys) + len(self.requestable_slots) + len(self.system_requestable_slots)
        else:
            if self.agent_role == 'system':
                self.NActions = 5 + len(self.ontology.ontology['system_requestable']) + len(self.ontology.ontology['requestable'])
                self.NOtherActions = 4 + 2 * len(self.ontology.ontology['requestable'])

            elif self.agent_role == 'user':
                self.NActions = 4 + 2 * len(self.ontology.ontology['requestable'])
                self.NOtherActions = 5 + len(self.ontology.ontology['system_requestable']) + len(self.ontology.ontology['requestable'])
    ...
    def next_action(self, state):
        ...
        if state_enc not in self.pi or (self.is_training and random.random() < self.epsilon):
            if not self.is_training:
                if not self.pi:
                    print(f'\nWARNING! WoLF-PHC pi is empty ({self.agent_role}). Did you load the correct file?\n')
                else:
            ...
            if random.random() < 0.75:
                ...
                # When randomly selecting actions, do not select the UNK() action (last index)!
                return self.decode_action(random.choice(range(0, self.NActions)), self.agent_role == 'system')

        if self.IS_GREEDY_POLICY:
            # Get greedy action
            max_pi = max(self.pi[state_enc][:-1])    # Do not consider 'UNK' etc
            ...
            if maxima:
                ...
                print('--- {0}: Warning! No maximum value identified for policy. Selecting random action.'.format(self.agent_role))
                # When randomly selecting actions, do not select the UNK() action (last index)!
                return self.decode_action(random.choice(range(0, self.NActions)), self.agent_role == 'system')
        else:
            # Sample next action
            sys_acts = self.decode_action(random.choices(range(len(self.pi[state_enc])), self.pi[state_enc])[0], self.agent_role == 'system')
    ...
    def encode_state(self, state):
        ...
        # return ''.join([str(b) for b in temp])
    ...
    def encode_action(self, actions, system=True):
        ...
        if not actions:
            print('WARNING: WoLF-PHC Policy action encoding called with empty actions list (returning -1).')
            return -1
        ...
        if system:
            ...
            if action.intent == 'inform':
                # return 3
                return len(self.dstc2_acts_usr) + len(self.requestable_slots) + self.system_requestable_slots.index(action.params[0].slot)

        if (self.agent_role == 'system') == system:
            print('WoLF-PHC ({0}) policy action encoder warning: Selecting default action (unable to encode: {1})!'.format(self.agent_role, action))
        else:
            print('WoLF-PHC ({0}) policy action encoder warning: Selecting default action (unable to encode other agent action: {1})!'.format(self.agent_role, action))
        ...
        if system:
            ...
            if action_enc < len(self.dstc2_acts_sys) + len(self.system_requestable_slots):
                return [DialogueAct('request',
                                    [DialogueActItem(self.system_requestable_slots[action_enc-len(self.dstc2_acts_sys)], Operator.EQ, '')])]
            ...
            if action_enc < len(self.dstc2_acts_usr) + len(self.requestable_slots):
                return [DialogueAct('request',
                                    [DialogueActItem(self.requestable_slots[action_enc-len(self.dstc2_acts_usr)], Operator.EQ, '')])]

            if action_enc < len(self.dstc2_acts_usr) + len(self.requestable_slots) + len(self.system_requestable_slots):
                return [DialogueAct('inform',
                                    [DialogueActItem(self.system_requestable_slots[action_enc-len(self.dstc2_acts_usr)-len(self.requestable_slots)], Operator.EQ, '')])]

        # Default fall-back action
        print('WoLF-PHC Policy ({0}) policy action decoder warning: Selecting repeat() action (index: {1})!'.format(self.agent_role, action_enc))
    ...
    def train(self, dialogues):
        ...
        for dialogue in dialogues:
            ...
            for turn in dialogue:
                ...
                action_enc = self.encode_action(turn['action'], self.agent_role == 'system')
                ...
                if state_enc not in self.pi:
                    self.pi[state_enc] = [float(1/self.NActions)] * self.NActions

                if state_enc not in self.mean_pi:
                    self.mean_pi[state_enc] = [float(1/self.NActions)] * self.NActions
                ...
                # Update Q
                self.Q[state_enc][action_enc] = ((1 - self.alpha) * self.Q[state_enc][action_enc]) + self.alpha * (
                            turn['reward'] + (self.gamma * np.max(self.Q[new_state_enc])))

                # Update mean policy estimate
                for a in range(self.NActions):
                    self.mean_pi[state_enc][a] = self.mean_pi[state_enc][a] + ((1.0 / self.state_counter[state_enc]) * (self.pi[state_enc][a] - self.mean_pi[state_enc][a]))
                ...
                for a in range(self.NActions):
                    sum_policy = sum_policy + (self.pi[state_enc][a] * self.Q[state_enc][a])
                    sum_mean_policy = sum_mean_policy + (self.mean_pi[state_enc][a] * self.Q[state_enc][a])
                ...
                for a in range(self.NActions):
                    if a == max_Q_idx:
                        self.pi[state_enc][a] = min(1.0, self.pi[state_enc][a] + d_plus)
                    else:
                        self.pi[state_enc][a] = max(0.0, self.pi[state_enc][a] + d_minus)
    ...
    def save(self, path=None):
        ...
        if self.statistics['total_turns'] > 0:
            print('DEBUG > {0} WoLF PHC Policy supervision ratio: {1}'.format(self.agent_role,
                                                                              float(self.statistics['supervised_turns'] / self.statistics['total_turns'])))

        print(f'DEBUG > {self.agent_role} WoLF PHC Policy state space size: {len(self.pi)}')
        ...
        if isinstance(path, str):
            if os.path.isfile(path):
                with open(path, 'rb') as file:
                    ...
                    print('WoLF-PHC Policy loaded from {0}.'.format(path))

            else:
                print('Warning! WoLF-PHC Policy file %s not found' % path)
        else:
            print('Warning! Unacceptable value for WoLF-PHC policy file name: %s ' % path)
