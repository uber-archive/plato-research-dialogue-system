...
class DummyNLU(NLU):
    def __init__(self, args):
        super(DummyNLU, self).__init__(args)
        ...
        if 'database' not in args:
            raise AttributeError('DummyNLU: Please provide database!')
        ...
        # In order to work for simulated users, we need access to possible values of requestable slots
        cursor = self.database.SQL_connection.cursor()

        print('DummyNLU: Preprocessing Database... (do not use DummyNLU with large databases!)')

        # Get table name
        db_result = cursor.execute("select * from sqlite_master where type = 'table';").fetchall()
        if db_result and db_result[0] and db_result[0][1]:
            ...
            all_items = cursor.execute("select * from " + db_table_name + ";").fetchall()
        ...
        # For this DummyNLU create a list of requestable-only to reduce computational load
        self.requestable_only_slots = [slot for slot in self.ontology.ontology['requestable'] if slot not in self.ontology.ontology['informable']] + ['name']
    ...
    def process_input(self, utterance, dialogue_state=None):
        ...
        if last_sys_act and last_sys_act.intent in ['request', 'expl-conf']:
            for p in self.dontcare_pattern:
                ...
                if p == utterance:
                    dact.intent = 'inform'
                    dact.params.append(DialogueActItem(last_sys_act.params[0].slot, Operator.EQ, 'dontcare'))
                    return [dact]
            ...
            if 'no info' in utterance:
                ...
                for slot in self.ontology.ontology['requestable']:
                    if slot in utterance:
                        dact.params.append(DialogueActItem(slot, Operator.EQ, 'no info'))
                        return [dact]

                # Else try to grab slot name from the other agent's previous act
                if last_sys_act and last_sys_act.intent in ['request', 'expl-conf']:
                    dact.params.append(DialogueActItem(last_sys_act.params[0].slot, Operator.EQ, 'dontcare'))
                    return [dact]
        ...
        if dact.intent in ['inform', 'request']:
            for word in words:
                # Check for requests. Requests for informable slots are captured below
                if word in self.requestable_only_slots:
                    if dact.intent == 'request':
                        dact.params.append(DialogueActItem(word, Operator.EQ, ''))
                        break
                        ...
                        # For any other kind of intent, we have no way of determining the slot's value, since such
                        # information is not in the ontology.
                        # dact.params.append(DialogueActItem(word, Operator.EQ, 'dontcare'))
                        # break

                # Check for informs (most intensive)
                if word in self.ontology.ontology['informable']:

                    # If a request intent has already been recognized, do not search for slot values
                    if dact.intent == 'request':
                        dact.params.append(DialogueActItem(word, Operator.EQ, ''))
                        break
                    ...
                    for p in self.ontology.ontology['informable'][word]:
                        ...
                        if match:
                            ...
                            dact.params.append(DialogueActItem(word, Operator.EQ, p))
                            found = True
                            break

                        # elif w in self.dontcare_pattern:
                        #     dact.intent = 'inform'
                        #     dact.params.append(DialogueActItem(word, Operator.EQ, 'dontcare'))
                        #     found = True
                        #     break
        ...
        if dact.intent in ['UNK', 'inform'] and not dact.params:
            ...
            for slot in slot_vals:
                for value in slot_vals[slot]:
                    if value and value.translate(self.punctuation_remover) in utterance:
                        if slot == 'name':
                            dact.intent = 'offer'
        ...
        # Check if something has been missed (e.g. utterance is dont care and there's no previous sys act)
        if dact.intent == 'inform':
            ...
            if dact.params:
                ...
                # Try to see if the utterance refers to a slot in the requestable ones (e.g. 'I prefer any phone')
                for slot in self.ontology.ontology['requestable']:
                    if slot in utterance:
                        # We can only handle 'dontcare' kind of values here, as we do not know values of req. slots.
                        for p in self.dontcare_pattern:
                            match = re.search(r'\b{0}\b'.format(p), utterance)
                            if match:
                                dact.params = [DialogueActItem(slot, Operator.EQ, 'dontcare')]
