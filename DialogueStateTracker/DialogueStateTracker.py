...
class DummyStateTracker(DialogueStateTracker):
    ...
    def __init__(self, args):
        super(DummyStateTracker, self).__init__(args)
        ...
        if result and result[0] and result[0][1]:
            ...
            raise ValueError(
                'Dialogue State Tracker cannot specify Table Name from database {0}'.format(self.database.db_file_name))

        self.DB_ITEMS = 0   # This will raise an error!

        self.domain = domain
        if domain in ['CamRest', 'SlotFilling']:
            self.DState = SlotFillingDialogueState({'slots': self.ontology.ontology['informable']})
        else:
            print('Warning! Domain has not been defined. Using Dummy Dialogue State')
            self.DState = DummyDialogueState({'slots': self.ontology.ontology['system_requestable']})
    ...
    def update_state(self, dacts):
        # TODO: These rules will create a field in the dialogue state slots filled dictionary if one doesn't exist.
        ...
        for dact in dacts:
            ...
            if dact.intent in ['inform', 'offer']:
                # The user provided new information so the system hasn't made any offers taking that into account yet.
                ...
                for dact_item in dact.params:
                    ...
                    elif self.DState.user_goal:# and self.DState.user_goal.actual_requests:
                        if dact_item.slot in self.DState.user_goal.actual_requests:
                            self.DState.user_goal.actual_requests[dact_item.slot].value = dact_item.value

                        # Only update requests that have been asked for
                        if dact_item.slot in self.DState.user_goal.requests:
                            self.DState.user_goal.requests[dact_item.slot].value = dact_item.value

            elif dact.intent == 'request':
                for dact_item in dact.params:
                    # TODO: THIS WILL ONLY SAVE THE LAST DACT ITEM! -- THIS APPLIES TO THE FOLLOWING RULES AS WELL
    ...
    def update_state_db(self, db_result=None, sys_req_slot_entropies=None, sys_acts=None):
        if db_result and sys_acts:
            raise ValueError('Dialogue State Tracker: Cannot update state as both system and user (i.e. please use only one argument as appropriate).')
        ...
        if db_result:
            ...
            if sys_req_slot_entropies:
                self.DState.system_requestable_slot_entropies = deepcopy(sys_req_slot_entropies)

            self.DState.db_result = db_result

        # This should be called if the agent is a user
        elif sys_acts:
            # Create dictionary if it doesn't exist or reset it if a new offer has been made
            if not self.DState.item_in_focus or 'offer' in [a.intent for a in sys_acts]:
                self.DState.item_in_focus = dict.fromkeys(self.ontology.ontology['requestable'])
                ...
                if sys_act.intent in ['inform', 'offer']:
                    for item in sys_act.params:
                        ...
                        if self.DState.user_goal:
                            if item.slot in self.DState.user_goal.actual_requests:
                                self.DState.user_goal.actual_requests[item.slot].value = item.value

                            # Only update requests that have been asked for
                            if item.slot in self.DState.user_goal.requests:
                                self.DState.user_goal.requests[item.slot].value = item.value
    ...
    def update_state_sysact(self, sys_acts):
        if sys_acts:
            ...
            for sys_act in sys_acts:
                ...
                # Keep track of actual requests made. These are used in reward and success calculation for systems. The
                # reasoning is that it does not make sense to penalise a system for an unanswered request that was
                # never actually made by the user.
                # If the current agent is a system then these will be disregarded.
                if sys_act.intent == 'request' and sys_act.params and self.DState.user_goal:
                    self.DState.user_goal.actual_requests[sys_act.params[0].slot] = sys_act.params[0]

                # Similarly, keep track of actual constraints made.
                if sys_act.intent == 'inform' and sys_act.params and self.DState.user_goal:
                    self.DState.user_goal.actual_constraints[sys_act.params[0].slot] = sys_act.params[0]
