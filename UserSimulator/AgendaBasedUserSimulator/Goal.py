...
class Goal:
    def __init__(self):
        ...
        self.ground_truth = None    # To be used in the multi-agent setting primarily
                                    # (where the user does not have access to the ground truth
                                    # - item in focus - in the dialogue state).
    ...
    def __str__(self):
        ...
        for c in self.constraints:
            ret += f'\t\tConstr({self.constraints[c].slot}={self.constraints[c].value})\n'
        ret += '\t\t-------------\n'
        ...
        for sg in self.subgoals:
            for c in sg.constraints:
                ...
                ret += f'\t\tConstr({sg.constraints[c].slot}={sg.constraints[c].value})\n'
            ret += '\t\t--------\n'
        ret += '\n'
...
class GoalGenerator:
    def __init__(self, ontology, database, goals_file=None):
        ...
        if result and result[0] and result[0][1]:
            ...
            raise ValueError('Goal Generator cannot specify Table Name from database {0}'.format(self.database.db_file_name))
        ...
        self.db_row_count = cursor.execute("SELECT COUNT(*) FROM " + self.db_table_name + ";").fetchall()[0][0]
    ...
    def generate(self, goal_slot_selection_weights=None):
        ...
        sql_command = "SELECT * FROM " + self.db_table_name + \
                      " WHERE ROWID == (" + str(random.randint(1, self.db_row_count)) + ");"
        ...
        while attempt < 3 and not db_result:
            print('GoalGenerator: Database {0} appears to be empty!'.format(self.database))
        ...
        if not db_result:
            raise LookupError('GoalGenerator: Database {0} appears to be empty!'.format(self.database))
        ...
        # TODO: Sample from all available operators, not just '=' (where applicable)
        ...
        if goal_slot_selection_weights:
            ...
            # Make sure that all slots have some weight. Flatten the dictionaries in the process
            slot_weights = []
            ...
            inf_slots = self.weighted_random_sample_no_replacement(list(self.ontology.ontology['informable'].keys()),
                                                                   slot_weights,
                                                                   random.randint(2, len(self.ontology.ontology['informable'])))

            inf_slots.reverse()
        else:
            inf_slots = random.sample(list(self.ontology.ontology['informable'].keys()),
                                      random.randint(2, len(self.ontology.ontology['informable'])))

        # Sample requests from requestable slots
        req_slots = random.sample(self.ontology.ontology['requestable'],
                                  random.randint(0, len(self.ontology.ontology['requestable'])))

        # Remove slots for which the user places constraints
        # Note: 'name' may or may not be in inf_slots here, and this is randomness is desirable
        ...
        # Shuffle informable and requestable slots to create some variety when pushing into the agenda.
        ...
        for slot in inf_slots:
            ...
            if slot in result and result[slot]:
                goal.constraints[slot] = DialogueActItem(slot, Operator.EQ, result[slot])
    ...
    # From: https://www.sciencedirect.com/science/article/pii/S002001900500298X
    def weighted_random_sample_no_replacement(self, population, weights, num_samples):
    ...
    def load_goals(self, path):
        ...
        if isinstance(goals_path, str):
            ...
            print(f'Warning! Unacceptable value for goals file name: {goals_path}')


# This generator will generate goals that have sub-goals
class ComplexGoalGenerator(GoalGenerator):
    def __init__(self, ontology, database, goals_file=None, global_key=None, global_slots=[], local_slots=[]):
        super(ComplexGoalGenerator, self).__init__(ontology, database, goals_file)

        self.global_key = global_key
        # TODO: What happens / what does it mean when no global key is provided? Fall back to regular GoalGenerator?

        self.global_slots = global_slots

        self.local_slots = local_slots
        if not self.local_slots:
            self.local_slots = list(self.ontology.ontology['informable'].keys())
    ...
    def generate(self, goal_slot_selection_weights=None):
        ...
        if not result:
            raise LookupError(f'ComplexGoalGenerator cannot find an item with global key {self.global_key}')

        # Generate goal
        goal = Goal()

        # Sample "global" constraints and requests that all sub-goals share
        # E.g. for uEats, this could be the cuisine or the price range
        global_inf_slots = random.sample(self.global_slots, random.randint(2, len(self.global_slots)))

        # Fetch all items from the database that satisfy the constraints sampled above
        sql_command = "SELECT * FROM " + self.db_table_name + \
                      " WHERE " + self.global_key + " = \"" + global_key_value + "\" AND "
        ...
        # Sample requests
        global_req_slots = random.sample(self.global_slots, random.randint(0, len(self.global_slots)))
        ...
        # num_sub_goals = random.randint(1, 4)
        num_sub_goals = random.choices(range(1, 5), weights=[0.15, 0.35, 0.35, 0.15])[0]

        subgoal_attempts = 0

        # As there is no guarantee that the sampled slots for the sampled subgoals exist (and we do not want empty subgoals),
        # make three attempts at sampling subgoals.
        while not goal.subgoals and subgoal_attempts < 3:
            results = random.sample(results, num_sub_goals)
            subgoal_attempts += 1

            # For each sub-goal, sample "local" constraints and requests (must be on different slots than the global)
            for sg in range(num_sub_goals):
                local_inf_slots = random.sample(self.local_slots, random.randint(1, len(self.local_slots)))
                ...
                for lif in local_inf_slots:
                    ...
                    if results[sg][lif] and results[sg][lif] not in ['None', 'Other']:
                        subgoal.constraints[lif] = DialogueActItem(lif, Operator.EQ, results[sg][lif])

                if subgoal.constraints:
                    goal.subgoals.append(subgoal)

        # TODO Check if a constraint exists in all subgoals, in which case remove from all subgoals and put in global constr
