...
class ConversationalMultiAgent(ConversationalAgent):
    ...
    def __init__(self, configuration, agent_id):
        ...
        if self.configuration:
            ...
            if 'NLU' in self.configuration[agent_id_str] and \
                ...
                elif self.configuration[agent_id_str]['NLU']['nlu'] == 'uEats':
                    self.nlu = uEatsNLU(nlu_args)
                    self.USE_NLU = True

                elif self.configuration[agent_id_str]['NLU']['nlu'] == 'RA':
            ...
            if 'NLG' in self.configuration[agent_id_str] and \
                ...
                elif self.configuration[agent_id_str]['NLG']['nlg'] == 'uEats':
                    self.nlg = uEatsNLG({})
    ...
    def start_dialogue(self, goal=None):
        ...
        #     self.dialogue_manager.get_state(), response, self.agent_goal
        # )
        # self.recorder.record(
        #     deepcopy(self.dialogue_manager.get_state()),
        #     self.dialogue_manager.get_state(), response, rew, success
        # )
    ...
    def continue_dialogue(self, args):
        ...
        if self.dialogue_turn < self.MAX_TURNS:
            ...
            #                     self.agent_goal.actual_requests[item.slot] =\
            #                         DialogueActItem(
            #                             item.slot,
            #                             Operator.EQ,
            #                             ''
            #                         )
            #                     self.dialogue_manager.update_goal(
            #                         self.agent_goal
            #                     )
            #
