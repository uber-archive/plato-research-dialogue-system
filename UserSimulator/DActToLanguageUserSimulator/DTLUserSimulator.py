...
class DTLUserSimulator(UserSimulator):
    def __init__(self, args):
        super(DTLUserSimulator, self).__init__(args)
        ...
        if 'policy_file' not in args:
            raise AttributeError('DTLUserSimulator: Please provide policy file!')
        ...
        if isinstance(policy_file, str):
            if os.path.isfile(policy_file):
                ...
                print('Warning! User Policy file %s not found' % policy_file)
        else:
            print('Warning! Unacceptable value for DTL Simulator policy file name: %s ' % policy_file)
    ...
    def receive_input(self, system_acts):
        ...
        if self.offer_made:
            ...
            meets_constraints = all([i.value == self.goal.constraints[i.slot].value for i in inform_dact.params if
                                     i.slot in self.goal.constraints])
            ...
            if meets_constraints:
                for item in inform_dact.params:
                    if item.slot in self.goal.actual_requests:
                        ...
                        # else:
                        #     print('WARNING! DTL User Simulator requested a slot not in the goal: {0}'.format(item.slot))

                # Check if all constraints and requests have been met
                # self.goal_met = True
                # for slot in self.goal.actual_requests:
                #     if not self.goal.actual_requests[slot].value:
                #         self.goal_met = False
                #         break
    ...
    def respond(self):
        ...
        if not self.input_system_acts:
            # Randomly sample from hello + responses to requests, as there is where informs most likely live.
            sys_act_slot = random.choice([act for act in self.policy if 'request' in act])

            replies = list(self.policy[sys_act_slot]['responses'].keys())
            probs = [self.policy[sys_act_slot]['responses'][i] for i in replies]
            ...
            for slot in self.ontology.ontology['informable']:
                if slot.upper() in response:
                    if slot in self.goal.constraints:
                        response = response.replace('<' + slot.upper() + '>', self.goal.constraints[slot].value)
                    else:
                        # If there is no constraint, replace with slot 'any'
                        response = response.replace('<' + slot.upper() + '>', 'any')

            for slot in self.ontology.ontology['requestable']:
                # This check is necessary to know when to mark this as an actual request
                if slot.upper() in response:
                    response = response.replace('<' + slot.upper() + '>', slot)

                    self.goal.actual_requests[slot] = DialogueActItem(slot, Operator.EQ, '')
        ...
        for system_act in self.input_system_acts:
            ...
            if sys_act_slot not in self.policy:
                print('Warning! DACT-NLG policy does not know what to do for %s' % sys_act_slot)
                # return ''
            else:
                replies = list(self.policy[sys_act_slot]['responses'].keys())
                probs = [self.policy[sys_act_slot]['responses'][i] for i in replies]
                ...
                for slot in self.ontology.ontology['requestable']:
                    ...
                    if slot.upper() in response:
                        if slot == 'addr':
                            response = response.replace('<' + slot.upper() + '>', 'address')
                        elif slot == 'postcode':
                            response = response.replace('<' + slot.upper() + '>', 'post code')
                        elif slot == 'pricerange':
                            response = response.replace('<' + slot.upper() + '>', 'price range')
                        else:
                            response = response.replace('<' + slot.upper() + '>', slot)
